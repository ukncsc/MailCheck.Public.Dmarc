...
namespace MailCheck.Dmarc.Entity.Entity.Notifiers
{
    public class AdvisoryChangedNotifier : IChangeNotifier
    {
        ...
        public AdvisoryChangedNotifier(
            ...
            IEqualityComparer<Contracts.SharedDomain.Message> messageEqualityComparer)
        {
        ...
        public void Handle(DmarcEntityState state, Message message)
        {
            if (message is DmarcRecordsEvaluated evaluationResult)
            {
                ...
                var currentRecordsMessages = state.DmarcRecords?.Messages.Where(_ => _.MessageDisplay != Contracts.SharedDomain.MessageDisplay.Prompt).ToList();
                if (currentRecordsMessages != null) currentMessages.AddRange(currentRecordsMessages);
                ...
                var newRecordsMessages = evaluationResult.Records?.Messages.Where(_ => _.MessageDisplay != Contracts.SharedDomain.MessageDisplay.Prompt).ToList();
                if (newRecordsMessages != null) newMessages.AddRange(newRecordsMessages);
                ...
                List<Contracts.SharedDomain.Message> sustainedMessages = currentMessages.Intersect(newMessages, _messageEqualityComparer).ToList();
                if (sustainedMessages.Any())
                {
                    DmarcAdvisorySustained dmarcAdvisorySustained = new DmarcAdvisorySustained(state.Id, sustainedMessages.Select(x => new AdvisoryMessage((MessageType)x.MessageType, x.Text, (MessageDisplay)x.MessageDisplay)).ToList());
                    _dispatcher.Dispatch(dmarcAdvisorySustained, _dmarcEntityConfig.SnsTopicArn);
                }
            }
        }
    }
