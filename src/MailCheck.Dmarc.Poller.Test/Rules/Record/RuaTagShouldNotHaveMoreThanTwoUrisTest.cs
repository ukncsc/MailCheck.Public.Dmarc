...
namespace MailCheck.Dmarc.Poller.Test.Rules.Record
{
    ...
    public class RuaTagShouldNotHaveMoreThanTwoUrisTest
    {
        ...
        public async Task MarkdownErrorMessage()
        {
            ...
            string expectedMarkdown = $"This DMARC record has requested mail servers to send aggregate reports to 3 destinations."
                + $"{Environment.NewLine}{Environment.NewLine}Please be aware that email servers are only obliged to deliver to the first two."
                + $"{Environment.NewLine}Any extra destinations may get a random selection of reports giving misleading statistics."
